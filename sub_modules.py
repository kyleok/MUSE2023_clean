...
class VariableSelectionNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.additional_context:
            ...
            # sparse_weights.transpose(-2,-1)
            combined = sparse_weights.transpose(-2, -1) * transformed_embedding
            # print('combined')
        ...
        # Static Inputs
            ...
            # flatten = torch.flatten(embedding, start_dim=1) #original
            ...
            sparse_weights = F.softmax(mlp_outputs, dim=-1)  # 여기까지 잘 작동하는거 봄
            sparse_weights = sparse_weights.unsqueeze(-2)
            #             print('sparse_weights')
            #             print(sparse_weights.shape)
            ...
            for i in range(self.output_size):
                ...
                e = self.per_feature_grn[i](embedding[:, :, :, i:i + 1].squeeze()).unsqueeze(-1)
                trans_emb_list.append(e)
