...
import config
from config import AROUSAL, PERSONALISATION_DIMS, PERSONALISATION, WANDB_ENTITY
...
def parse_args():
    ...
    parser.add_argument('--feature', required=True, help='feature')
    ...
    args.use_gpu = True
    args.normalize = True if args.normalize != 0 else False
    ...
    if args.predict:
        if args.eval_personalised:
            args.paths['predict'] = os.path.join(config.PREDICTION_FOLDER, PERSONALISATION, 'personalised',
                                                 args.emo_dim,
    ...
    args.model_file = os.path.join(config.BEST_MODEL_FOLDER, args.model_id,
...
def eval_personalised(personalised_cps: Dict[str, str], id2data_loaders: Dict[str, Dict[str, DataLoader]],
                      use_gpu=False):
    """
...
def eval_trained_checkpoints(paths, feature, emo_dim, normalize, win_len, hop_len, cp_dir, use_gpu):
    ...
    checkpoints = sorted(
        [cp for cp in glob(os.path.join(escape(cp_dir), 'model_*.pth')) if 'initial' not in os.path.basename(cp)])
...
def personalise(model, feature, emo_dim, temp_dir, paths, normalize, win_len, hop_len, epochs, lr, use_gpu, loss_fn,
    ...
    personalised_cps = train_personalised_models(model=model, temp_dir=temp_dir, data_loaders=data_loaders,
                                                 subject_ids=test_ids, epochs=epochs,
                                                 lr=lr, use_gpu=use_gpu, loss_fn=loss_fn, eval_fn=eval_fn,
                                                 eval_metric_str=eval_metric_str,
...
def log_personalisation_results(csv_path, params, val_score, test_score, metric_name,
                                exclude_keys=['result_csv', 'cache', 'save', 'save_path', 'predict', 'eval_model',
                                              'log_file_name']):
    """
...
if __name__ == '__main__':
    args = parse_args()
    wandb.init(entity=WANDB_ENTITY, project='7_3_best_personals')
    wandb.run.name = f"{args.normalize}_{args.win_len}_{args.hop_len}_{args.epochs}_{args.batch_size}_{args.lr}_{args.seed}_{args.n_seeds}_{args.early_stopping_patience}_{args.reduce_lr_patience}_{args.regularization}"
    ...
    if not args.eval_personalised:
        ...
        val_score, test_score = personalise(model=model, feature=args.feature, emo_dim=args.emo_dim, temp_dir=pers_dir,
                                            paths=args.paths,
                                            normalize=args.normalize, win_len=args.win_len, hop_len=args.hop_len,
                                            epochs=args.epochs, lr=args.lr,
                                            use_gpu=args.use_gpu, loss_fn=loss_fn,
                                            eval_fn=eval_fn, eval_metric_str=eval_metric_str,
                                            early_stopping_patience=args.early_stopping_patience,
                                            reduce_lr_patience=args.reduce_lr_patience,
                                            regularization=args.regularization, seeds=seeds)
